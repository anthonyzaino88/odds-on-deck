  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <Link
          href="/games"
          className="inline-flex items-center text-[11px] font-medium uppercase tracking-wide text-slate-500 hover:text-slate-300 transition-colors"
        >
          ← Today's Slate
        </Link>
      ...
      <div>
        ...
        <p className="text-xs text-slate-500 uppercase tracking-wide font-mono tabular-nums">
          {game.away.abbr} @ {game.home.abbr}
        </p>

        {/* Score Display */}
        {(game.status === 'in_progress' || (game.status === 'final' && game.homeScore != null)) ? (
          <div className="mt-3 bg-surface border border-white/[0.06] rounded-[4px] p-4 max-w-sm">
            <div className="flex items-center justify-between mb-2">
              <span className="text-sm font-semibold text-slate-100 tabular-nums">{game.away.abbr}</span>
              <span className={cn(
                'text-xl font-semibold tabular-nums font-mono',
                game.status === 'in_progress' ? 'text-slate-100' :
            ...
            <div className="flex items-center justify-between">
              <span className="text-sm font-semibold text-slate-100 tabular-nums">{game.home.abbr}</span>
              <span className={cn(
                'text-xl font-semibold tabular-nums font-mono',
                game.status === 'in_progress' ? 'text-slate-100' :
                ...
                <>
                  ...
                  {!isNHL && game.inning && (
                    <span className="text-xs text-slate-400 tabular-nums">
                      ...
                      {(game.balls !== null || game.strikes !== null) && ` • ${game.balls || 0}-${game.strikes || 0}`}
                    </span>
                  )}
                </>
              ) : (
                <span className="text-[11px] font-semibold text-slate-500 uppercase tracking-wider">Final</span>
              )}
            </div>
            {game.status === 'in_progress' && game.lastPlay && (
              <p className="text-[11px] text-slate-500 mt-2">{game.lastPlay}</p>
            )}
          </div>
        ) : (
          <div className="mt-3">
            <span className="inline-flex items-center px-1.5 py-0.5 rounded-[3px] text-[10px] font-semibold uppercase tracking-wide bg-amber-500/10 text-amber-400 border border-amber-500/20">
      ...
      {isNFL && game.nflData && (
        <div className="card">
          <div className="px-4 py-3 border-b border-white/[0.06]">
            <h2 className="text-[11px] font-semibold uppercase tracking-widest text-slate-500">Game Details</h2>
          </div>
          <div className="p-4">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div>
                <span className="text-sm font-medium text-slate-400">Status:</span>
                <span className="ml-2 text-sm text-slate-100 capitalize">{formatGameStatus(game.status)}</span>
              </div>
              {game.nflData.quarter && (
                <div>
                  <span className="text-sm font-medium text-slate-400">Quarter:</span>
                  <span className="ml-2 text-sm text-slate-100">Q{game.nflData.quarter}</span>
                </div>
              )}
              {game.nflData.timeLeft && (
                <div>
                  <span className="text-sm font-medium text-slate-400">Time Left:</span>
                  <span className="ml-2 text-sm text-slate-100">{game.nflData.timeLeft}</span>
                </div>
              )}
              {game.nflData.lastPlay && (
                <div className="md:col-span-3">
                  <span className="text-sm font-medium text-slate-400">Last Play:</span>
                  <span className="ml-2 text-sm text-slate-100">{game.nflData.lastPlay}</span>
                </div>
              )}
              {!game.nflData.quarter && !game.nflData.timeLeft && game.status === 'in_progress' && (
                <div className="md:col-span-3">
                  <p className="text-sm text-slate-400 italic">Live game data updating...</p>
                </div>
              )}
            </div>
          </div>
      ...
      {isNHL && game.status === 'final' && game.postGameStats && (() => {
        ...
        return (
          <div className="card">
            <div className="px-4 py-3 border-b border-white/[0.06]">
              <h2 className="text-[11px] font-semibold uppercase tracking-widest text-slate-500">Post-Game Wrap-Up</h2>
            ...
            <div className="p-4">
              {/* Score Summary */}
              <div className="mb-6 p-4 bg-elevated/40 rounded-[4px] border border-white/[0.06]">
                <div className="flex items-center justify-between">
                  <div className="text-center flex-1">
                    <div className="text-2xl font-bold text-slate-100">{game.away.abbr}</div>
                    <div className="text-3xl font-bold text-blue-400 mt-2">{game.awayScore || 0}</div>
                  </div>
                  <div className="text-slate-400 mx-4">vs</div>
                  <div className="text-center flex-1">
                    <div className="text-2xl font-bold text-slate-100">{game.home.abbr}</div>
                    <div className="text-3xl font-bold text-blue-400 mt-2">{game.homeScore || 0}</div>
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                {awayStats && (
                  <div className="space-y-4">
                    <h3 className="text-sm font-semibold text-slate-100 border-b border-white/[0.06] pb-2">
                      {awayStats.teamName || game.away.name} Statistics
                    </h3>
                    <div className="grid grid-cols-2 gap-3">
                      {awayStats.shotsOnGoal !== null && (
                        <div>
                          <span className="text-xs text-slate-400">Shots on Goal</span>
                          <div className="text-lg font-semibold text-slate-100">{awayStats.shotsOnGoal}</div>
                        </div>
                      )}
                      {awayStats.powerPlayGoals !== null && awayStats.powerPlayOpportunities !== null && (
                        <div>
                          <span className="text-xs text-slate-400">Power Play</span>
                      ...
                      {awayStats.faceoffWins !== null && awayStats.faceoffTotal !== null && (
                        <div>
                          <span className="text-xs text-slate-400">Faceoffs</span>
                          <div className="text-lg font-semibold text-slate-100">
                      ...
                      {awayStats.hits !== null && (
                        <div>
                          ...
                          <div className="text-lg font-semibold text-slate-100">{awayStats.hits}</div>
                        </div>
                      )}
                      {awayStats.blockedShots !== null && (
                        <div>
                          <span className="text-xs text-slate-400">Blocked Shots</span>
                          <div className="text-lg font-semibold text-slate-100">{awayStats.blockedShots}</div>
                        </div>
                      )}
                      {awayStats.takeaways !== null && (
                        <div>
                          <span className="text-xs text-slate-400">Takeaways</span>
                          <div className="text-lg font-semibold text-slate-100">{awayStats.takeaways}</div>
                        </div>
                      )}
                      {awayStats.giveaways !== null && (
                        <div>
                          <span className="text-xs text-slate-400">Giveaways</span>
                          <div className="text-lg font-semibold text-slate-100">{awayStats.giveaways}</div>
                        </div>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      {homeStats.shotsOnGoal !== null && (
                        <div>
                          <span className="text-xs text-slate-400">Shots on Goal</span>
                          <div className="text-lg font-semibold text-slate-100">{homeStats.shotsOnGoal}</div>
                        </div>
                      )}
                      ...
                      {homeStats.hits !== null && (
                        <div>
                          <span className="text-xs text-slate-400">Hits</span>
                          <div className="text-lg font-semibold text-slate-100">{homeStats.hits}</div>
                        </div>
                      )}
                      {homeStats.blockedShots !== null && (
                        <div>
                          <span className="text-xs text-slate-400">Blocked Shots</span>
                          <div className="text-lg font-semibold text-slate-100">{homeStats.blockedShots}</div>
                        </div>
                      )}
                      {homeStats.takeaways !== null && (
                        <div>
                          <span className="text-xs text-slate-400">Takeaways</span>
                          <div className="text-lg font-semibold text-slate-100">{homeStats.takeaways}</div>
                        </div>
                      )}
                      {homeStats.giveaways !== null && (
                        <div>
                          <span className="text-xs text-slate-400">Giveaways</span>
                          <div className="text-lg font-semibold text-slate-100">{homeStats.giveaways}</div>
                        </div>
                      )}
                    </div>
                  </div>
              ...
              {/* Game Summary */}
              {homeStats && awayStats && (
                <div className="mt-6 p-4 bg-elevated/30 rounded-[4px] border border-white/[0.06]">
                  <h4 className="text-sm font-semibold text-slate-100 mb-3">Game Summary</h4>
                  <div className="space-y-2 text-sm text-slate-300">
                    ...
                    {homeStats.powerPlayOpportunities !== null && awayStats.powerPlayOpportunities !== null && (
                      <div className="flex justify-between">
                        <span className="text-slate-400">Total Power Plays:</span>
                        <span className="text-slate-100 font-medium">
              ...
              {game.postGamePlayerStats && (() => {
                ...
                return (
                  <div className="mt-6 space-y-6">
                    ...
                    {topScorers.length > 0 && (
                      <div>
                        <h4 className="text-sm font-semibold text-slate-100 mb-3 border-b border-white/[0.06] pb-2">
                          Top Scorers
                        </h4>
                          ...
                          <table className="w-full text-sm">
                            <thead>
                              <tr className="border-b border-white/[0.06]">
                                ...
                                <th className="text-center py-2 text-slate-400 font-medium">Team</th>
                                <th className="text-center py-2 text-slate-400 font-medium">G</th>
                                <th className="text-center py-2 text-slate-400 font-medium">A</th>
                                <th className="text-center py-2 text-slate-400 font-medium">PTS</th>
                                <th className="text-center py-2 text-slate-400 font-medium">SOG</th>
                              </tr>
                            </thead>
                            <tbody>
                              {topScorers.map((player, idx) => (
                                <tr key={idx} className="border-b border-white/[0.06] hover:bg-elevated/30">
                                  <td className="py-2 text-slate-100 font-medium">{player.playerName}</td>
                                  <td className="py-2 text-center text-slate-300">{player.teamAbbr}</td>
                                  <td className="py-2 text-center text-slate-100 font-semibold">{player.goals}</td>
                                  <td className="py-2 text-center text-slate-300">{player.assists}</td>
                                  <td className="py-2 text-center text-blue-400 font-bold">{player.points}</td>
                                  <td className="py-2 text-center text-slate-400">{player.shots}</td>
                                </tr>
                              ))}
                            </tbody>
                          </table>
                      ...
                      <div>
                        ...
                        <div className="overflow-x-auto">
                          <table className="w-full text-sm">
                            <thead>
                              <tr className="border-b border-white/[0.06]">
                                <th className="text-left py-2 text-slate-400 font-medium">Goalie</th>
                                <th className="text-center py-2 text-slate-400 font-medium">Team</th>
                                <th className="text-center py-2 text-slate-400 font-medium">SV</th>
                                <th className="text-center py-2 text-slate-400 font-medium">GA</th>
                            ...
                            <tbody>
                              {gameGoalies.map((goalie, idx) => (
                                <tr key={idx} className="border-b border-white/[0.06] hover:bg-elevated/30">
                                  <td className="py-2 text-slate-100 font-medium">{goalie.playerName}</td>
                                  <td className="py-2 text-center text-slate-300">{goalie.teamAbbr}</td>
                                  <td className="py-2 text-center text-slate-100 font-semibold">{goalie.saves}</td>
                                  <td className="py-2 text-center text-slate-300">{goalie.goalsAgainst !== null ? goalie.goalsAgainst : '-'}</td>
                                  {goalie.savePercentage !== null && (
                                    <td className="py-2 text-center text-blue-400 font-semibold">
                      ...
                      <div>
                        ...
                        <div className="space-y-2">
                          {penaltyLeaders.map((player, idx) => (
                            <div key={idx} className="flex items-center justify-between p-2 bg-elevated/30 rounded">
                              <div>
                                <span className="text-slate-100 font-medium">{player.playerName}</span>
                                <span className="text-slate-400 ml-2">({player.teamAbbr})</span>
                              </div>
                  ...
                  return (
                    <div className="mt-6 p-4 bg-red-500/[0.08] border border-red-500/20 rounded-[4px]">
                      <p className="text-sm text-red-400">Error displaying player statistics</p>
                    </div>
                  )
                }
              })()}
      ...
      {isNHL && (
        <div className="card">
          <div className="px-4 py-3 border-b border-white/[0.06]">
            <h2 className="text-[11px] font-semibold uppercase tracking-widest text-slate-500">Game Information</h2>
          </div>
          <div className="p-4">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div>
                <span className="text-sm font-medium text-slate-400">Status:</span>
                <span className="ml-2 text-sm text-slate-100 capitalize">
            ...
            {game.status === 'scheduled' && (
              <div className="mt-4 p-4 bg-surface border border-white/[0.06] rounded-[4px]">
                <p className="text-sm text-slate-300">
                  <strong className="text-slate-100 font-semibold">Game Scheduled:</strong> This game is scheduled for {(() => {
                    // Use exact same approach as slate page
                    const dateStr = game.date || ''
            ...
            {game.status === 'in_progress' && (
              <div className="mt-4 p-4 bg-green-500/[0.08] border border-green-500/20 rounded-[4px]">
                <p className="text-sm text-green-400">
                  <strong className="font-semibold">Game in Progress:</strong> Live updates are being fetched.
                </p>
              </div>
            )}
          </div>
      ...
      <div className="card">
        <div className="px-4 py-3 border-b border-white/[0.06]">
          <h2 className="text-[11px] font-semibold uppercase tracking-widest text-slate-500">Recent Odds</h2>
          <p className="text-sm text-slate-400 mt-1">
        ...
        <div className="p-4">
          {isNHL && (!game.odds || game.odds.length === 0) ? (
            <div className="text-center py-8">
              ...
              <p className="text-sm text-slate-400 mb-2">No odds data available yet</p>
              <p className="text-xs text-slate-500 mb-4">
                Odds are typically available 24-48 hours before game time
              </p>
              <div className="bg-surface border border-white/[0.06] rounded-[4px] p-4 mt-4 text-left">
                <p className="text-xs font-medium text-slate-300 mb-2">To fetch odds for this game:</p>
                <code className="block text-xs bg-bg text-slate-300 p-2 rounded mt-2">
...
function StatCard({ title, value, subtitle, className = 'text-slate-100' }) {
  return (
    <div className="bg-surface border border-white/[0.06] rounded-[4px] p-4">
      <h3 className="text-[11px] font-semibold text-slate-500 uppercase tracking-widest mb-1">
        {title}
      </h3>
      <p className={`text-2xl font-semibold tabular-nums font-mono ${className}`}>{value}</p>
  ...
  return (
    <>
      ...
      <div className="card p-4 lg:col-span-2">
        <h3 className="text-xs font-medium text-slate-400 uppercase tracking-wide mb-3">Moneyline</h3>
        <div className="flex items-center gap-3 mb-2">
          <span className="text-sm font-semibold text-slate-100 w-10">{game.away?.abbr}</span>
          <div className="flex-1 bg-slate-700 rounded-full h-5 overflow-hidden flex">
            <div
              className="bg-blue-500 h-full rounded-l-full flex items-center justify-center text-[10px] font-bold text-slate-100 transition-all"
              style={{ width: `${awayProb ? (awayProb * 100) : 50}%`, minWidth: '30px' }}
            >
              {awayProb ? `${(awayProb * 100).toFixed(0)}%` : '—'}
            </div>
            <div
              className="bg-amber-500 h-full rounded-r-full flex items-center justify-center text-[10px] font-bold text-slate-100 transition-all"
              style={{ width: `${homeProb ? (homeProb * 100) : 50}%`, minWidth: '30px' }}
            >
              {homeProb ? `${(homeProb * 100).toFixed(0)}%` : '—'}
            </div>
          </div>
          <span className="text-sm font-semibold text-slate-100 w-10 text-right">{game.home?.abbr}</span>
        </div>
        <div className="flex justify-between text-xs text-slate-400 px-12">
      ...
      <div className="card p-4">
        ...
        {edge ? (
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              <span className="text-xs text-slate-400">{game.away?.abbr}</span>
            ...
            <div className="flex items-center justify-between">
              <span className="text-xs text-slate-400">{game.home?.abbr}</span>
      ...
      {/* Total / O/U */}
      <div className="card p-4">
        <h3 className="text-xs font-medium text-slate-400 uppercase tracking-wide mb-2">Total (O/U)</h3>
        {totalOdds?.total ? (
          <div className="text-center">
            <p className="text-2xl font-bold text-slate-100">{totalOdds.total.toFixed(1)}</p>
            <p className="text-xs text-slate-400 mt-1">{totalOdds.book || 'Latest'}</p>
          </div>
        ) : (
      ...
      <div className="card p-4">
        <h3 className="text-xs font-medium text-slate-400 uppercase tracking-wide mb-2">Venue</h3>
        <div className="space-y-3">
          <div>
            <span className="text-xs text-slate-400">Park Factor</span>
    ...
    <div className="card p-4">
      ...
      <div className="space-y-3">
        ...
        {pitcher ? (
          <>
            <div>
              <span className="text-sm font-medium text-slate-400">Pitcher:</span>
              <span className="ml-2 text-sm text-slate-100">{pitcher.fullName}</span>
            </div>
            <div>
              <span className="text-sm font-medium text-slate-400">Throws:</span>
              <span className="ml-2 text-sm text-slate-100">
          ...
          <div className="text-sm text-slate-400">Probable pitcher TBD</div>
        )}
      </div>
    </div>
  )
    ...
    <div className="card p-4">
      ...
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1 space-y-2">
          <div>
            <span className="text-sm font-medium text-slate-400">Team:</span>
            <span className="ml-2 text-sm text-slate-100">{team.name} ({team.abbr})</span>
          </div>
          {pitcher ? (
            <>
              <div>
                <span className="text-sm font-medium text-slate-400">Pitcher:</span>
                <span className="ml-2 text-sm font-semibold text-slate-100">{pitcher.fullName}</span>
                <span className="ml-2 text-xs text-slate-400">({pitcher.throws}HP)</span>
              </div>
              {pitcher.record && (
                <div>
                  <span className="text-sm font-medium text-slate-400">Record:</span>
                  <span className="ml-2 text-sm text-slate-100">{pitcher.record}</span>
                </div>
              )}
              {pitcher.era && (
                <div>
                  <span className="text-sm font-medium text-slate-400">ERA:</span>
                  <span className="ml-2 text-sm text-slate-100">{pitcher.era}</span>
                </div>
              )}
            </>
          ) : (
...
function MLBInsightsSection({ game, edge }) {
  ...
  return (
    <div className="card">
      <div className="px-4 py-3 border-b border-white/[0.06]">
        <h2 className="text-[11px] font-semibold uppercase tracking-widest text-slate-500">Betting Insights</h2>
        <p className="text-xs text-slate-500 mt-0.5">Key takeaways for this matchup</p>
      </div>
      <div className="p-4 space-y-4">
        {insights.map((insight, idx) => (
          <div key={idx} className="flex items-start gap-2.5 p-3 bg-elevated/40 rounded-[4px] border border-white/[0.06]">
            <span className="mt-1.5 h-1.5 w-1.5 rounded-full bg-green-500/60 shrink-0" />
...
function MLBPlayerPropsSection({ props }) {
  ...
  return (
    <div className="card">
      <div className="px-4 py-3 border-b border-white/[0.06]">
        <h2 className="text-[11px] font-semibold uppercase tracking-widest text-slate-500">Player Props</h2>
      ...
      {props.length === 0 ? (
        <div className="p-4 text-center">
          <p className="text-slate-400 text-sm">No player props available yet</p>
          <p className="text-slate-500 text-xs mt-1">Props are typically available 12-24 hours before game time</p>
        </div>
      ) : null}
      <div className="p-4 space-y-6" style={props.length === 0 ? {display: 'none'} : undefined}>
        {battingProps.length > 0 && (
          <div>
            <h3 className="text-[11px] font-semibold uppercase tracking-widest text-slate-500 mb-3 flex items-center gap-2">Batting Props <span className="text-slate-600 font-mono tabular-nums">{battingProps.length}</span></h3>
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-white/[0.06]">
                    <th className="text-left py-2 text-slate-400 font-medium">Player</th>
                    <th className="text-left py-2 text-slate-400 font-medium">Prop</th>
                    <th className="text-center py-2 text-slate-400 font-medium">Line</th>
                    <th className="text-center py-2 text-slate-400 font-medium">Odds</th>
                    <th className="text-center py-2 text-slate-400 font-medium">Win %</th>
                    <th className="text-center py-2 text-slate-400 font-medium">Quality</th>
                  </tr>
                </thead>
                <tbody>
                  {battingProps.slice(0, 15).map((prop, idx) => (
                    <tr key={prop.propId || idx} className="border-b border-white/[0.06] hover:bg-elevated/30">
                      <td className="py-2 text-slate-100 font-medium">{prop.playerName}</td>
                      <td className="py-2 text-slate-300">{prop.pick?.toUpperCase()} {(prop.type || '').replace(/_/g, ' ').replace('batter ', '')}</td>
                      <td className="py-2 text-center text-slate-100">{prop.threshold}</td>
                      <td className="py-2 text-center text-amber-400 font-semibold">{decimalToAmerican(prop.odds) || '—'}</td>
                      <td className="py-2 text-center">
                        <span className={`font-semibold ${(prop.probability || 0) >= 0.55 ? 'text-green-400' : (prop.probability || 0) >= 0.50 ? 'text-blue-400' : 'text-slate-400'}`}>
                          {((prop.probability || 0) * 100).toFixed(0)}%
                        </span>
                      </td>
                      <td className="py-2 text-center text-slate-400">{prop.qualityScore?.toFixed(1) || '—'}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        {pitchingProps.length > 0 && (
          <div>
            <h3 className="text-[11px] font-semibold uppercase tracking-widest text-slate-500 mb-3 flex items-center gap-2">Pitching Props <span className="text-slate-600 font-mono tabular-nums">{pitchingProps.length}</span></h3>
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-white/[0.06]">
                ...
                <tbody>
                  {pitchingProps.slice(0, 10).map((prop, idx) => (
                    <tr key={prop.propId || idx} className="border-b border-white/[0.06] hover:bg-elevated/30">
                      <td className="py-2 text-slate-100 font-medium">{prop.playerName}</td>
                      <td className="py-2 text-slate-300">{prop.pick?.toUpperCase()} {(prop.type || '').replace(/_/g, ' ').replace('pitcher ', '')}</td>
                      <td className="py-2 text-center text-slate-100">{prop.threshold}</td>
                      <td className="py-2 text-center text-amber-400 font-semibold">{decimalToAmerican(prop.odds) || '—'}</td>
                      <td className="py-2 text-center">
                        <span className={`font-semibold ${(prop.probability || 0) >= 0.55 ? 'text-green-400' : (prop.probability || 0) >= 0.50 ? 'text-blue-400' : 'text-slate-400'}`}>
...
function MLBBoxScoreSection({ boxScore, game }) {
  function TeamBoxScore({ teamData, teamName, teamAbbr }) {
    ...
    return (
      <div className="space-y-4">
        <h3 className="text-sm font-semibold text-slate-100 border-b border-white/[0.06] pb-2">{teamName} ({teamAbbr})</h3>
        {teamData.batters.length > 0 && (
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-white/[0.06]">
                  <th className="text-left py-1.5 text-slate-400 font-medium">Batter</th>
                  <th className="text-center py-1.5 text-slate-400 font-medium w-10">AB</th>
                  <th className="text-center py-1.5 text-slate-400 font-medium w-10">R</th>
                  <th className="text-center py-1.5 text-slate-400 font-medium w-10">H</th>
                  <th className="text-center py-1.5 text-slate-400 font-medium w-10">RBI</th>
                  <th className="text-center py-1.5 text-slate-400 font-medium w-10">BB</th>
                  <th className="text-center py-1.5 text-slate-400 font-medium w-10">SO</th>
                  <th className="text-center py-1.5 text-slate-400 font-medium w-10">HR</th>
                </tr>
              </thead>
              <tbody>
                {teamData.batters.map((b, idx) => (
                  <tr key={idx} className="border-b border-white/[0.06] hover:bg-elevated/30">
                    <td className="py-1.5">
                      ...
                      <span className="text-slate-500 text-xs ml-1">{b.position}</span>
                    </td>
                    <td className="py-1.5 text-center text-slate-300">{b.ab}</td>
                    <td className="py-1.5 text-center text-slate-300">{b.r}</td>
                    <td className="py-1.5 text-center text-slate-100 font-semibold">{b.h}</td>
                    <td className="py-1.5 text-center text-slate-300">{b.rbi}</td>
                    <td className="py-1.5 text-center text-slate-300">{b.bb}</td>
                    <td className="py-1.5 text-center text-slate-300">{b.so}</td>
                    <td className="py-1.5 text-center font-semibold">{b.hr > 0 ? <span className="text-amber-400">{b.hr}</span> : <span className="text-slate-500">0</span>}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        {teamData.pitchers.length > 0 && (
          <div className="overflow-x-auto mt-3">
            <div className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2">Pitching</div>
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-white/[0.06]">
                  <th className="text-left py-1.5 text-slate-400 font-medium">Pitcher</th>
                  <th className="text-center py-1.5 text-slate-400 font-medium w-10">IP</th>
                  <th className="text-center py-1.5 text-slate-400 font-medium w-10">H</th>
                  <th className="text-center py-1.5 text-slate-400 font-medium w-10">R</th>
                  <th className="text-center py-1.5 text-slate-400 font-medium w-10">ER</th>
                  <th className="text-center py-1.5 text-slate-400 font-medium w-10">BB</th>
                  <th className="text-center py-1.5 text-slate-400 font-medium w-10">SO</th>
                  <th className="text-center py-1.5 text-slate-400 font-medium w-10">PC</th>
                </tr>
              </thead>
              <tbody>
                {teamData.pitchers.map((p, idx) => (
                  <tr key={idx} className="border-b border-white/[0.06] hover:bg-elevated/30">
                    <td className="py-1.5 text-slate-100 font-medium">{p.name}</td>
                    <td className="py-1.5 text-center text-slate-300">{p.ip}</td>
                    <td className="py-1.5 text-center text-slate-300">{p.h}</td>
                    <td className="py-1.5 text-center text-slate-300">{p.r}</td>
                    <td className="py-1.5 text-center text-slate-300">{p.er}</td>
                    <td className="py-1.5 text-center text-slate-300">{p.bb}</td>
                    <td className="py-1.5 text-center text-slate-100 font-semibold">{p.so}</td>
                    <td className="py-1.5 text-center text-slate-400">{p.pitches}</td>
                  </tr>
                ))}
              </tbody>
            </table>
  ...
  return (
    <div className="card">
      <div className="px-4 py-3 border-b border-white/[0.06]">
        <h2 className="text-[11px] font-semibold uppercase tracking-widest text-slate-500">Box Score</h2>
        <p className="text-xs text-slate-500 mt-0.5">Final game statistics</p>
      </div>
...
function PitcherMatchupSection({ analysis, game }) {
  ...
  function SplitStatRow({ label, data, statKeys }) {
    ...
    return (
      <tr className="hover:bg-elevated">
        <td className="px-3 py-2 text-sm font-medium text-slate-300">{label}</td>
        {statKeys.map(key => (
          <td key={key} className="px-3 py-2 text-sm text-center text-slate-100">
  ...
  function PitcherSplitsCard({ splits, pitcherName, teamAbbr, opponentName, opponentAbbr }) {
    if (!splits?.overall) return (
      <div className="text-center py-6 text-slate-500 text-sm">No split data available</div>
    )
    ...
    return (
      <div>
        <div className="flex items-center justify-between mb-3">
          <h4 className="font-medium text-slate-100">{pitcherName} ({teamAbbr})</h4>
          {splits.season && <span className="text-xs text-slate-500">{splits.season} season</span>}
        </div>

        {vsOpponent && (
          <div className="mb-4 bg-amber-500/[0.06] border border-amber-500/20 rounded-[4px] p-3">
        ...
        {(splits.platoon?.vsLeft || splits.platoon?.vsRight) && (
          <div className="mt-4">
            <h5 className="text-xs font-medium text-slate-400 mb-2 uppercase tracking-wider">Opponent Batting by Handedness</h5>
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-white/[0.06]">
                <thead className="bg-surface">
                  <tr>
        ...
        {splits.timesThrough?.firstTime && (
          <div className="mt-4">
            <h5 className="text-xs font-medium text-slate-400 mb-2 uppercase tracking-wider">Times Through Order</h5>
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-white/[0.06]">
                <thead className="bg-surface">
                  <tr>
    ...
    <div className="space-y-8">
      ...
      {predictor?.homeTeam && (
        <div className="bg-surface rounded-[4px] p-4">
          <h4 className="text-xs font-medium text-slate-400 mb-3 uppercase tracking-wider">ESPN Win Probability</h4>
          <div className="flex items-center gap-3">
            <span className="text-sm text-slate-300 w-12 text-right">{game.away?.abbr}</span>
            ...
            <span className="text-sm text-slate-300 w-12">{game.home?.abbr}</span>
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {/* Last 5 */}
        <div className="bg-surface rounded-[4px] p-4">
          <h4 className="text-xs font-medium text-slate-400 mb-2 uppercase tracking-wider">Last 5 Games</h4>
          {last5?.length > 0 ? last5.map((team, i) => (
            <div key={i} className="mb-2">
              <span className="text-sm font-medium text-slate-100">{team.teamAbbr}: </span>
        ...
        {/* Season Series */}
        <div className="bg-surface rounded-[4px] p-4">
          <h4 className="text-xs font-medium text-slate-400 mb-2 uppercase tracking-wider">Season Series</h4>
          {seasonSeries?.length > 0 ? (() => {
            ...
            return (
              <div className="space-y-2">
                {current && (
                  <div>
                    <div className="text-xs text-slate-500">This Series ({current.totalGames || 3} games)</div>
                    <div className="text-sm text-slate-100 font-medium">{current.summary}</div>
                  </div>
                )}
                {season && season.summary !== current?.summary && (
                  <div>
                    <div className="text-xs text-slate-500">Season ({season.totalGames || '—'} games)</div>
                    <div className="text-sm text-slate-100 font-medium">{season.summary}</div>
                  </div>
                )}
                {!current && !season && seasonSeries.map((s, i) => (
                  <div key={i} className="text-sm text-slate-300">{s.summary || s.title}</div>
                ))}
              </div>
            )
          })() : (
            <p className="text-sm text-slate-500">No series history yet</p>
          )}
        </div>

        {/* Weather */}
        <div className="bg-surface rounded-[4px] p-4">
          <h4 className="text-xs font-medium text-slate-400 mb-2 uppercase tracking-wider">Weather</h4>
          {weather ? (
            <>
              <div className="text-sm text-slate-100">{weather.displayValue || weather.temperature + '°F'}</div>
              {weather.conditionId && (
                <div className="text-xs text-slate-400 mt-1">Condition: {weather.conditionId}</div>
              )}
            </>
          ) : (
            <p className="text-sm text-slate-500">Not available yet</p>
...
function BattingLineupTable({ game }) {
  if (!game.lineups || game.lineups.length === 0) {
    return (
      <div className="text-center py-8">
        <p className="text-slate-500">Lineups not yet available</p>
        <p className="text-sm text-slate-400 mt-1">
        ...
        {awayLineup.length > 0 ? (
          <div className="space-y-2">
            {awayLineup.map((lineup) => (
              <div key={lineup.id} className="flex items-center justify-between p-3 bg-surface rounded-[4px]">
                <div className="flex items-center space-x-3">
                  <span className="text-sm font-semibold text-slate-500 tabular-nums font-mono w-6">
                    {lineup.battingOrder}
                  </span>
                  <div>
          ...
          <p className="text-slate-500 italic">Lineup not yet announced</p>
        )}
      </div>
...
function OddsTable({ odds, isNFL, isNHL }) {
  if (!odds || odds.length === 0) {
    return <div className="text-sm text-slate-400">No odds data available</div>
  }
  ...
  return (
    <div className="space-y-6">
      {h2hOdds.length > 0 && (
        <div>
          <h4 className="font-medium text-slate-100 mb-3">Moneyline</h4>
          <div className="overflow-x-auto">
            <table className="min-w-full">
              <thead>
                <tr className="border-b border-white/[0.06]">
                  <th className="text-left text-xs font-medium text-slate-400 uppercase py-2">
                    Book
                  </th>
                  <th className="text-left text-xs font-medium text-slate-400 uppercase py-2">
                    Away
              ...
              <tbody className="divide-y divide-white/[0.06]">
                {h2hOdds.map((odds, idx) => (
                  <tr key={idx}>
                    <td className="py-2 text-sm text-slate-100">{odds.book}</td>
                    <td className="py-2 text-sm text-slate-100">
      ...
      {(isNFL || isNHL) && spreadOdds.length > 0 && (
        <div>
          <h4 className="font-medium text-slate-100 mb-3">Spread</h4>
          <div className="overflow-x-auto">
            <table className="min-w-full">
              <thead>
                <tr className="border-b border-white/[0.06]">
              ...
              <tbody className="divide-y divide-white/[0.06]">
                {spreadOdds.map((odds, index) => (
                  <tr key={index}>
                    <td className="py-2 text-sm text-slate-100">{odds.book}</td>
                    <td className="py-2 text-sm font-medium text-slate-100">
      ...
      {totalOdds.length > 0 && (
        <div>
          <h4 className="font-medium text-slate-100 mb-3">Totals</h4>
          <div className="overflow-x-auto">
            <table className="min-w-full">
              <thead>
                <tr className="border-b border-white/[0.06]">
