...
function featuredSportsFor(sport) {
  const key = String(sport || '').toLowerCase().trim()
  if (key === 'mixed') return [...PUBLISHED_SPORTS]
  if (PUBLISHED_SPORTS.includes(key)) return [key]
  return []
}

/**
 * Upcoming / unexpired / not-stale cache rows for Featured.
 * Same SQL prefilter as the public Published desk — juice + odds band
 * still run in JS via isPublishedEligibleProp.
 */
async function fetchFeaturedPublishedProps(sport, gameId = null) {
  const sports = featuredSportsFor(sport)
  if (!supabase || sports.length === 0) return []
  ...
  if (error) {
    console.error('❌ Featured: failed to load Published-eligible props', error)
    return []
  }

  return (data || [])
    .map((prop) => mapCachePropToParlayBet({
      ...prop,
    ...
    .filter(isFeaturedQualityLeg)
}

/**
 * Featured path only. Props that clear isPublishedEligibleProp, exactly
 * 3 legs, MLB + NFL. No sides / totals. Empty when the slate is thin.
 */
async function generateFeaturedPublishedParlays({
  sport,
  type,
  legCount,
  maxParlays,
  ...
  if (legCount !== FEATURED_LEG_COUNT) {
    console.log(`✅ Featured: ${legCount}-leg requested — Featured is ${FEATURED_LEG_COUNT} legs or empty`)
    return []
  }

  const sports = featuredSportsFor(sport)
  if (sports.length === 0) {
    console.log(`✅ Featured: ${sport} is outside Published sports (MLB + NFL)`)
    return []
  }

  const bets = await fetchFeaturedPublishedProps(sport, gameId)
  if (bets.length < FEATURED_LEG_COUNT) {
    console.log(`✅ Featured: only ${bets.length} Published-eligible props — empty > filler`)
    return []
  }
  ...
  if (parlays.length > 0) {
    const top = parlays[0]
    console.log(`🏆 Featured Published ${FEATURED_LEG_COUNT}-leg: ${(top.probability * 100).toFixed(1)}% win, ${top.legs.length} Published-eligible props`)
  } else {
    console.log('✅ Featured: no 3-leg Published-eligible card (empty > filler)')
  }
...
export async function generateSimpleParlays(options = {}) {
  const {
    ...
    gameId = null,
    featured = false, // Featured cards: Published-eligible 3-leg only, or empty
  } = options
  ...
  try {
    ...
    if (featured) {
      return await generateFeaturedPublishedParlays({
        sport,
        type,
        legCount,
        maxParlays,
