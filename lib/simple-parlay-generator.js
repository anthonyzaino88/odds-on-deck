...
export async function generateSimpleParlays(options = {}) {
  ...
  try {
    ...
    console.log(`🎮 Active game IDs (${activeGameIds.length}):`, activeGameIds.slice(0, 5))
    
    // Check if requested game is in the list
    if (gameId) {
      const isInList = activeGameIds.includes(gameId)
      console.log(`🔍 Requested game "${gameId}":`, isInList ? '✅ IN LIST' : '❌ NOT IN LIST')
      
      if (!isInList) {
        // Find the game to see its status
        ...
        if (requestedGame) {
          console.log(`   Game exists in DB: status="${requestedGame.status}", sport=${requestedGame.sport}, date=${requestedGame.date}`)
          console.log(`   Active statuses: scheduled, in_progress, in-progress, halftime`)
        } else {
    ...
    if (activeGameIds.length === 0) {
      console.log('⚠️ No active games found for parlays')
      return []
    }
    
    // Step 2: Fetch player props from Supabase PlayerPropCache - only from active games
    let playerProps = []
    try {
      let query = supabase
        .from('PlayerPropCache')
        .select('*')
        .in('gameId', activeGameIds) // Only props from active games
        .eq('isStale', false)
    ...
    playerProps.forEach(prop => {
      ...
      // Player props use bookmaker probabilities (edge ~0%), so use lower threshold
      // Quality is determined by teamContext, confidence, and probability instead
      const effectiveMinEdge = prop.edge !== undefined ? -0.05 : minEdge // Allow props with -5% to +5% edge
      
      // DEBUG: Check why props are being filtered out
      if (propsConsidered === 1) {
        console.log(`   🔍 Filtering thresholds: minEdge=${effectiveMinEdge}, minConfidence=${minConfidence} (level ${minConfidenceLevel})`)
        console.log(`   🔍 First prop: edge=${prop.edge}, confidence=${prop.confidence} (level ${propConfidenceLevel}), sport=${prop.sport}`)
        console.log(`   🔍 Pass edge check? ${prop.edge >= effectiveMinEdge}, Pass confidence check? ${propConfidenceLevel >= minConfidenceLevel}`)
      }
      
      if (prop.edge >= effectiveMinEdge && propConfidenceLevel >= minConfidenceLevel) {
        ...
        if (sport === 'mixed' || propSport === sport) {
          ...
          // DEBUG: Check what teamContext looks like
          if (propsConsidered === 1) {
            console.log(`   🔍 First prop teamContext:`, ctx ? 'EXISTS' : 'MISSING', ctx ? Object.keys(ctx).length + ' keys' : '')
            if (ctx) {
              console.log(`      Keys: ${Object.keys(ctx).slice(0, 5).join(', ')}...`)
              console.log(`      Sample values: offensiveRating=${ctx.offensiveRating}, isHotOffense=${ctx.isHotOffense}`)
            }
          }
