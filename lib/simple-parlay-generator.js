...
export async function generateSimpleParlays(options = {}) {
  ...
  try {
    ...
    // HONEST EDGE SYSTEM: Sort based on filter mode
    ...
    if (filterMode === 'safe') {
      ...
      // Home run mode: highest payout (odds) while maintaining reasonable probability
      // Filter to only parlays with at least 5% win chance, then sort by payout
      sortedParlays = sortedParlays
        .filter(p => p.probability >= 0.05)
        .sort((a, b) => b.totalOdds - a.totalOdds)
    } else {
      // Default: win probability (safest)
      sortedParlays = sortedParlays.sort((a, b) => b.probability - a.probability)
    }
