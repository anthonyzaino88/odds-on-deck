...
/** Canonical hash of the on-disk ESPN summary object. Not part of observationContentHash. */
export function rawContentHash(raw) {
  if (raw == null || typeof raw !== 'object') return null
  return stableHash(raw)
}

/**
 * Season/week/seasonType may have been copied from scoreboard discovery rather
 * than the summary raw. Re-parse using the stored values so audits do not
 * false-fail healthy archives whose summary omitted those fields.
 */
export function archiveDiscoveredMetadata(record) {
  if (!record || typeof record !== 'object') return null
  return {
    season: record.season ?? null,
    week: record.week ?? null,
    season_type: record.season_type ?? record.seasonType ?? null,
  }
}

function playerArchiveKey(player) {
  if (player?.provider_player_id) return `id:${player.provider_player_id}`
  return `name:${player?.player_name || ''}::team:${player?.provider_team_id || ''}`
}

export function diffNormalizedPlayerStats(storedPlayers, fromRawPlayers) {
  const mismatches = []
  const fromRaw = new Map((fromRawPlayers || []).map((p) => [playerArchiveKey(p), p]))
  const seen = new Set()
  for (const player of storedPlayers || []) {
    const key = playerArchiveKey(player)
    seen.add(key)
    const counterpart = fromRaw.get(key)
    if (!counterpart) {
      mismatches.push({
        player: player.player_name,
        field: '*',
        stored: player.stats || null,
        from_raw: null,
      })
      continue
    }
    const fields = new Set([
      ...
      ...Object.keys(counterpart.stats || {}),
    ])
    for (const field of fields) {
      if (player.stats?.[field] !== counterpart.stats?.[field]) {
        mismatches.push({
          player: player.player_name,
          field,
          stored: player.stats?.[field] ?? null,
          from_raw: counterpart.stats?.[field] ?? null,
        })
      }
    }
  }
  for (const player of fromRawPlayers || []) {
    const key = playerArchiveKey(player)
...
/**
 * Re-parse raw with stored scoreboard metadata and compare observation fields
 * (teams, players/stats, game_time, completion). Season/week/seasonType are
 * taken from the stored record so separately sourced scoreboard values match.
 */
export function compareRawToNormalized(raw, normalized) {
  const reparsed = parseEspnNflSummary(raw, {
    fetchedAt: normalized?.fetched_at,
    discovered: archiveDiscoveredMetadata(normalized),
  })
  if (!reparsed) {
    return {
      ok: false,
      mismatches: [{ kind: 'raw_unparseable_summary' }],
      from_raw_hash: null,
      stored_hash: observationContentHash(normalized),
      stat_mismatches: [],
    }
  }
  const storedHash = observationContentHash(normalized)
  const fromRawHash = observationContentHash(reparsed)
  const statMismatches = diffNormalizedPlayerStats(normalized?.players, reparsed.players)
  const ok = storedHash === fromRawHash
  return {
    ok,
    mismatches: ok
      ? []
      : [
          {
            kind: 'observation_hash',
            stored: storedHash,
            from_raw: fromRawHash,
          },
          ...statMismatches.slice(0, 12),
        ],
    from_raw_hash: fromRawHash,
    stored_hash: storedHash,
    stat_mismatches: statMismatches,
  }
}
...
export function verifyNflArchiveVersion(archiveRoot, row) {
  ...
  let recomputedRawHash = null
  if (rawStat.missing) {
    issues.push({ kind: 'missing_raw', version: row.version, path: paths.raw })
  } else if (rawStat.corrupt || rawStat.value == null || typeof rawStat.value !== 'object') {
    ...
    recomputedRawHash = rawContentHash(rawStat.value)
    if (!recomputedRawHash) {
      issues.push({ kind: 'corrupt_raw', version: row.version, path: paths.raw })
    }
  }
  ...
  if (normStat.missing) {
    ...
    const storedRawHash = row.raw_hash || normalized.raw_hash
    if (storedRawHash && recomputedRawHash && storedRawHash !== recomputedRawHash) {
      issues.push({
        kind: 'raw_hash_mismatch',
        version: row.version,
        stored: storedRawHash,
        recomputed: recomputedRawHash,
      })
    }
    if (rawStat.value && !rawStat.missing && !rawStat.corrupt) {
      const consistency = compareRawToNormalized(rawStat.value, normalized)
      if (!consistency.ok) {
        issues.push({
          kind: 'raw_normalized_mismatch',
          version: row.version,
          stored: consistency.stored_hash,
          from_raw: consistency.from_raw_hash,
          stat_mismatches: (consistency.stat_mismatches || []).slice(0, 12),
        })
      }
    }
  ...
  const corrupt = issues.some(
    (i) =>
      ...
      i.kind === 'raw_hash_mismatch' ||
      i.kind === 'raw_normalized_mismatch'
  )
...
function indexRecordFrom(normalized, paths, version) {
  return {
    ...
    raw_hash: normalized.raw_hash || null,
...
export function writeNflBoxScoreArchive({
  ...
  const rawStat = statJsonFile(paths.raw)
  const rawHash = rawContentHash(rawStat.value)
  if (rawStat.missing || rawStat.corrupt || !rawHash) {
