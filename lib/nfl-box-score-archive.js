...
export function filterCompletedNflEvents(events, { from, to } = {}) {
  // Date-only --from/--to are inclusive UTC calendar days.
  // `new Date('YYYY-MM-DD')` is 00:00Z, which would drop afternoon/evening
  // kickoffs on the --to day (and disagrees with dateListInclusive).
