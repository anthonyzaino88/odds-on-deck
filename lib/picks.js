...
function buildNflGameLinePicksFromSnapshots({ game, edge, oddsRows, minEdge }) {
  ...
  if (selection.moneyline?.eligibility?.eligibleForPublic) {
    ...
    if (!match || !snapshotsAreCompatible({ ...mlQuote, gameId: game.id, market: 'h2h' }, match).ok) {
      return []
    }
  }
  ...
  if (selection.totals?.eligibility?.eligibleForPublic) {
    const match = selectCompatibleQuote(oddsRows, {
      ...
      gameId: game.id,
      market: 'totals',
      line: totalsQuote?.line,
    })
    if (!match || !snapshotsAreCompatible({
      ...totalsQuote,
      gameId: game.id,
      market: 'totals',
      line: totalsQuote?.line,
    }, { ...match, line: match.total ?? match.line }).ok) {
      return []
    }
  }

  return toPublicNflGameLines(selection, { minEdge }).map((line) => ({
    ...line,
    gameTime: game.date,
    status: game.status,
    sport: 'nfl',
