...
import {
  isCurrentNflSelectionModel,
  isLegacyNflHeuristicVersion,
...
export async function generatePicksFromSupabase(sport, gameId = null, minEdge = GAME_LINE_MIN_EDGE) {
  try {
    ...
    for (const game of games) {
      ...
      const built = buildGameLinePicksFromSnapshots({
        game,
        edge,
        oddsRows: oddsData,
        minEdge,
...
/**
 * Pure builder: turn a stored EdgeSnapshot + Odds rows into public
 * game-line picks. NFL uses the stored model contract and rejects
 * legacy / mismatched snapshots. MLB keeps the historical reconstruction.
 */
export function buildGameLinePicksFromSnapshots({
  game,
  edge,
  oddsRows = [],
  minEdge = GAME_LINE_MIN_EDGE,
} = {}) {
  if (!game || !edge) return []
  const sport = String(game.sport || '').toLowerCase()
  if (sport === 'nfl') {
    return buildNflGameLinePicksFromSnapshots({ game, edge, oddsRows, minEdge })
  }
  return buildReconstructedGameLinePicks({
    game,
    edge,
    latestOdds: latestOddsByMarket(oddsRows),
    minEdge,
  })
}

function latestOddsByMarket(oddsRows) {
  const latestOdds = {}
  for (const odd of Array.isArray(oddsRows) ? oddsRows : []) {
...
function buildNflGameLinePicksFromSnapshots({ game, edge, oddsRows, minEdge }) {
  if (isLegacyNflHeuristicVersion(edge.modelRun)) return []
  if (!isCurrentNflSelectionModel(edge.modelRun)) return []
  if (!nflGameLineSnapshotIsPublic(edge)) return []

  const selection = parseEdgePayload(edge)
  if (!selection) return []

  const mlQuote = selection.moneyline?.quote
  let moneylineQuoteOk = true
  if (selection.moneyline?.eligibility?.eligibleForPublic) {
    const match = selectCompatibleQuote(oddsRows, {
      ...mlQuote,
      gameId: game.id,
      market: 'h2h',
    })
    moneylineQuoteOk = Boolean(
      match && snapshotsAreCompatible({ ...mlQuote, gameId: game.id, market: 'h2h' }, match).ok,
    )
  }

  const totalsQuote = selection.totals?.quote
  let totalsQuoteOk = true
  if (selection.totals?.eligibility?.eligibleForPublic) {
    const match = selectCompatibleQuote(oddsRows, {
      ...totalsQuote,
      gameId: game.id,
      market: 'totals',
      line: totalsQuote?.line ?? totalsQuote?.total,
    })
    totalsQuoteOk = Boolean(
      match && snapshotsAreCompatible({
        ...totalsQuote,
        gameId: game.id,
        market: 'totals',
        line: totalsQuote?.line ?? totalsQuote?.total,
  ...
  return toPublicNflGameLines(selection, { minEdge })
    .filter((line) => {
      if (line.type === 'moneyline') return moneylineQuoteOk
      if (line.type === 'total') return totalsQuoteOk
      return false
    })
    .map((line) => ({
      ...line,
...
/**
 * MLB (and any non-NFL sport still on EdgeSnapshot floats): reconstruct
 * from stored edge + latest quote. Unchanged on purpose.
 */
function buildReconstructedGameLinePicks({ game, edge, latestOdds, minEdge }) {
  const picks = []
