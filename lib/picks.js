 ...
 */
export async function generateEditorPicks(filterMode = 'value') {
  try {
    const picks = []
    ...
    // Hard filter: counting-stat under 0.5 juice traps never make Editor's Picks
    let filteredPicks = picks.filter(p => !isJuiceTrap(p))
    if (filterMode === 'safe') {
      ...
      // Value mode: line-shop edge, 3+ books when the row actually stores a book count
      filteredPicks = filteredPicks.filter(p => {
        const edge = p.edge || 0
        const isGamePick = p.type === 'moneyline' || p.type === 'total'
        const books = getBookCount(p)
        if (books != null && books < 3) return false
        // Game picks: 5%+ edge, Player props: 10%+ edge
        return isGamePick ? edge >= 0.05 : edge >= 0.10
      })
    }
    // 'all' mode returns everything (minus juice traps)
    
    // Rank by line-shop edge (not qualityScore / favorite-finder).
    // Safe mode keeps probability as the sort so that filter remains usable.
    filteredPicks.sort((a, b) => comparePicksByRank(a, b, filterMode))
...
/**
 * Generate picks from player props cache, ranked by line-shop edge.
 * Juice traps are dropped here and again in generateEditorPicks.
 */
  ...
  try {
    ...
    const picks = props
      ...
      .map(prop => ({
        ...
        reasoning: [prop.playerName, (prop.pick || '').toUpperCase(), prop.threshold, (prop.type || '').replace(/_/g, ' ')].join(' '),
    ...
    console.log('Found ' + picks.length + ' player prop picks after juice-trap filter')
    return picks
...
/**
 * Default ranker is line-shop edge, not qualityScore (favorite-finder).
 * Safe mode keeps probability so the high-prob filter remains a distinct view.
 */
function comparePicksByRank(a, b, filterMode) {
  if (filterMode === 'safe') {
