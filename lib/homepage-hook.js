...
function mapBoardRow({ prop, source }) {
  ...
  const why = source === 'editors'
    ? (edge != null && edge > 0
      ? `+${edge.toFixed(1)}% edge`
      : "Editor's look — not on the Published track")
    : (edge != null && edge > 0
      ? `+${edge.toFixed(1)}% edge`
      : null)
...
export async function getHomepageProofStats() {
  try {
    return await getPublishedPicksStats()
  } catch (error) {
    console.error('homepage-hook: published stats failed', error)
