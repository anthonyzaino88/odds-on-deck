...
export function featuredSnapshotKeyFor(sport, type, now = new Date()) {
  return featuredSnapshotKey({ sport, type, legs: [] }, now)
}
...
function snapshotRowTime(row) {
  const instant = Date.parse(row?.createdAt || row?.generatedAt || 0)
  return Number.isFinite(instant) ? instant : 0
}

/**
 * First Featured-cleared write for a sport+kind+ET-day slot wins.
 * Tie-break on id so concurrent inserts are deterministic.
 */
export function featuredSnapshotWinner(rows) {
  const list = (Array.isArray(rows) ? rows : []).filter((row) => row && row.id)
  if (list.length === 0) return null
  return [...list].sort((a, b) => {
    const timeDiff = snapshotRowTime(a) - snapshotRowTime(b)
    if (timeDiff !== 0) return timeDiff
    return String(a.id).localeCompare(String(b.id))
  })[0]
}

export function isFeaturedPendingStatus(row) {
  const state = String(row?.outcome || row?.status || 'pending').toLowerCase()
  return !SETTLED.has(state) && state !== 'cancelled'
}

/**
 * Later copies of the same snapshot key. Persist only retracts pending
 * losers so a settled grade is never deleted by a race.
 */
export function featuredDuplicateRows(rows) {
  const winner = featuredSnapshotWinner(rows)
  if (!winner) return []
  return (Array.isArray(rows) ? rows : []).filter((row) => row && row.id && row.id !== winner.id)
}

export function planFeaturedDuplicateCleanup(rows) {
  const groups = new Map()
  for (const row of Array.isArray(rows) ? rows : []) {
    const key = parseFeaturedSnapshotKey(row?.notes)
    if (!key) continue
  ...
  for (const [key, group] of groups) {
    const winner = featuredSnapshotWinner(group)
    if (!winner) continue
    keep.push({ key, row: winner })
    for (const extra of featuredDuplicateRows(group)) {
      retract.push({ key, row: extra, winnerId: winner.id })
    }
  }
  return { keep, retract }
}

export function featuredPendingDuplicates(rows) {
  return featuredDuplicateRows(rows).filter(isFeaturedPendingStatus)
}

export function featuredPersistClaim(insertedId, rows) {
  const winner = featuredSnapshotWinner(rows)
  const retract = featuredPendingDuplicates(rows)
  return {
    winner,
    retract,
    keepInserted: Boolean(winner && insertedId && winner.id === insertedId),
  }
}

/**
 * One row per snapshot key for history / ROI. First write wins.
 * Rows without a parseable key stay (they are not slot dupes).
 */
export function dedupeFeaturedCohortRows(rows) {
  const list = filterFeaturedCohortRows(rows)
  ...
  for (const row of list) {
    ...
    const winner = featuredSnapshotWinner([current, row])
    bestByKey.set(key, winner)
  }
  const winners = new Set([...bestByKey.values()].map((row) => row.id))
  return list.filter((row) => {
    const key = parseFeaturedSnapshotKey(row?.notes)
    if (!key) return true
    return winners.has(row.id)
  })
}

export function featuredLegSignature(parlay) {
  return (parlay?.legs || [])
    .map((leg) => [
      String(leg.playerName || '').trim().toLowerCase(),
      String(leg.propType || leg.type || '').trim().toLowerCase(),
      String(leg.selection || leg.pick || '').trim().toLowerCase(),
...
/**
 * Page cards: keep SGP before multi, then one card per snapshot key
 * and per identical leg signature so a multi request that collapsed
 * into today's SGP cannot render twice.
 */
export function dedupeFeaturedPageCards(cards) {
  ...
  for (const card of list) {
    const snapshot = card?.parlay?.snapshotKey
      || parseFeaturedSnapshotKey(card?.parlay?.notes)
    const identity = snapshot || `${card?.sport}-${card?.type}`
    const legs = featuredLegSignature(card?.parlay)
    if (seenKeys.has(identity)) continue
    if (legs && seenLegs.has(legs)) continue
    seenKeys.add(identity)
    if (legs) seenLegs.add(legs)
    out.push(card)
  }
  return out
}

export function featuredRowToDisplayParlay(row) {
  if (!row || typeof row !== 'object') return null
  const legs = [...(row.legs || [])]
    .sort((a, b) => (Number(a.legOrder) || 0) - (Number(b.legOrder) || 0))
    .map((leg) => ({
      ...leg,
      gameId: leg.gameId || leg.gameIdRef,
  ...
  return {
    ...row,
    totalOdds: Number(row.totalOdds) || 1,
    probability: Number(row.probability) || 0,
    expectedValue: Number(row.expectedValue) || 0,
    edge: Number(row.edge) || 0,
    type: row.type || (new Set(legs.map((leg) => leg.gameId)).size === 1 ? 'single_game' : 'multi_game'),
    snapshotKey: parseFeaturedSnapshotKey(row.notes),
    legs,
  }
}
...
export function featuredHistoryLegActual(leg) {
  ...
  if (isNumericFeaturedActual(stored)) return stored
 ...
 * After #23 the regraded ParlayLeg.outcome is the card truth.
 * Joined PropValidation.result / actualValue can still disagree
 * (Otton PV correct+2 vs leg lost Actual 3; Goff PV incorrect on a hit).
 * 1. stored ParlayLeg.outcome
 * 2. stored Actual: N, then joined actual, vs line + over/under
 ...
 * Dots follow ParlayLeg.outcome. Shown actual prefers the stored
 * "Actual: N" the regrade wrote, not a disagreeing PV join.
 */
export function attachFeaturedHistoryLegDisplay(leg, validations = []) {
  ...
  const actualFromRow = usable && isNumericFeaturedActual(validation?.actualValue)
    ...
    : null
  const actualValue = actualFromStored ?? actualFromRow
...
export function summarizeFeaturedParlays(parlays) {
  const rows = dedupeFeaturedCohortRows(parlays)
