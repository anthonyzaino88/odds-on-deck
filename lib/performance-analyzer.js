...
export async function analyzePerformance() {
  try {
    // Get all completed validations using pagination (Supabase has 1000 row limit)
    let allValidations = []
    let page = 0
    const pageSize = 1000
    let hasMore = true
    ...
    while (hasMore) {
      ...
      // Stable ordering is REQUIRED for reliable range pagination — without it
      // a short page can end the loop early and undercount the dataset.
    ...
    validations.forEach(v => {
      if (!byPropType[v.propType]) {
        byPropType[v.propType] = { correct: 0, total: 0, incorrect: 0, pushes: 0, avgEdge: 0, edges: [] }
      }
      byPropType[v.propType].total++
      if (v.result === 'correct') byPropType[v.propType].correct++
      else if (v.result === 'incorrect') byPropType[v.propType].incorrect++
      else if (v.result === 'push') byPropType[v.propType].pushes++
      if (v.edge != null && !isNaN(v.edge)) {
    ...
    Object.keys(byPropType).forEach(propType => {
      ...
      // Only count validations with actual results (correct or incorrect, excluding pushes and nulls)
      const withResults = stat.correct + stat.incorrect
      const accuracy = withResults > 0 ? stat.correct / withResults : 0
      const avgEdge = stat.edges.length > 0 
        ? stat.edges.reduce((a, b) => (a || 0) + (b || 0), 0) / stat.edges.length 
        : 0
    ...
    validations.forEach(v => {
      if (!byPlayer[v.playerName]) {
        byPlayer[v.playerName] = { correct: 0, incorrect: 0, total: 0 }
      }
      byPlayer[v.playerName].total++
      if (v.result === 'correct') byPlayer[v.playerName].correct++
      else if (v.result === 'incorrect') byPlayer[v.playerName].incorrect++
    })
    ...
    validations.forEach(v => {
      ...
      else if (v.result === 'incorrect') bySource[source].incorrect++
    })
    ...
    Object.keys(byStrategy).forEach(strategy => {
      ...
      const propsWithResults = props.filter(p => p.result === 'correct' || p.result === 'incorrect')
      if (propsWithResults.length >= 5) {
        const correct = props.filter(p => p.result === 'correct').length
        const accuracy = propsWithResults.length > 0 ? correct / propsWithResults.length : 0
    ...
    const completedWithResults = validations.filter(v => v.result === 'correct' || v.result === 'incorrect')
    const correctCount = validations.filter(v => v.result === 'correct').length
    const overallAccuracy = completedWithResults.length > 0 
      ? correctCount / completedWithResults.length 
      : 0
