 ...
 *
 * Featured is Published-eligible props only — the same
 * isPublishedEligibleProp / PUBLISHED_* bar as the public card. No
 * forked prob/EV Featured definition. Empty is better than filler.
 */
...
/** Featured cards are exactly this many legs, or empty. Same floor as today's board. */
export const FEATURED_LEG_COUNT = TODAYS_BOARD_MIN_PUBLISHED
...
/**
 * Map a parlay leg onto the Published record shape so Featured cannot
 * fork thresholds. Game-line betType becomes type so isGameLineRecord fires.
 */
export function featuredLegAsPublishedRecord(leg) {
  if (!leg || typeof leg !== 'object') return null
  const market = leg.type || leg.propType || leg.betType
  const pick = leg.pick || leg.selection
  return {
    ...leg,
    pick,
    prediction: leg.prediction || pick,
    type: market,
    propType: leg.propType || market,
    odds: leg.odds,
    edge: leg.edge,
    qualityScore: leg.qualityScore,
    sport: leg.sport,
...
/**
 * Same freshness spirit as public Published rows: upcoming / unexpired /
 * not-stale. Missing fields (unit-test fixtures) do not fail the check.
 */
export function isFeaturedFreshLeg(leg, now = new Date()) {
  if (!leg || typeof leg !== 'object') return false
  if (leg.isStale === true) return false
  ...
  if (leg.expiresAt) {
    const exp = leg.expiresAt instanceof Date ? leg.expiresAt : new Date(leg.expiresAt)
    if (!Number.isNaN(exp.getTime()) && exp.getTime() <= nowMs) return false
  }

  if (leg.gameTime) {
    const at = leg.gameTime instanceof Date ? leg.gameTime : new Date(leg.gameTime)
    if (!Number.isNaN(at.getTime()) && at.getTime() <= nowMs) return false
  }
...
/**
 * Featured quality IS Published eligibility. One bar — do not add
 * prob/EV/juice forks here.
 */
export function isFeaturedQualityLeg(leg, now = new Date()) {
  if (!isFeaturedFreshLeg(leg, now)) return false
  return isPublishedEligibleProp(featuredLegAsPublishedRecord(leg))
}

export function isFeaturedWorthyParlay(parlay, now = new Date()) {
  if (!parlay || !Array.isArray(parlay.legs)) return false
  if (parlay.legs.length !== FEATURED_LEG_COUNT) return false
  if (!combinationPassesIntegrity(parlay.legs, parlay.type || 'multi_game')) return false
  return parlay.legs.every((leg) => isFeaturedQualityLeg(leg, now))
}
...
export function uniqueGameCount(legs) {
  return new Set(
    (Array.isArray(legs) ? legs : [])
      .map((leg) => leg?.gameId || leg?.gameIdRef)
      .filter(Boolean)
  ).size
}

/**
 * Featured SGP and multi are different snapshot slots. A same-game trio
 * must not fill the multi slot (that is how /parlays rendered two SGPs).
 * Explorer assemble does not use this.
 */
export function matchesFeaturedRequestedType(parlay, requestedType) {
  const games = uniqueGameCount(parlay?.legs)
  const requested = String(requestedType || parlay?.type || 'multi_game').toLowerCase()
  if (requested === 'single_game') return games === 1
  if (requested === 'multi_game' || requested === 'cross_sport') return games >= 2
  return games > 0
}
...
function summedLegEdge(parlay) {
  return (parlay?.legs || []).reduce((sum, leg) => sum + (Number(leg.edge) || 0), 0)
}

function rankParlays(parlays, filterMode, featured) {
  const ranked = [...parlays]
  if (featured) {
    // Rank among Published-eligible cards by the same edge-then-QS spirit
    // as the public board. EV is a tiebreak only — not a Featured bar.
    ranked.sort((a, b) => {
      const edgeDiff = summedLegEdge(b) - summedLegEdge(a)
      if (Math.abs(edgeDiff) > 1e-9) return edgeDiff
      const qs = (b.qualityScore || 0) - (a.qualityScore || 0)
      if (Math.abs(qs) > 0.1) return qs
...
export function assembleParlays(bets, options = {}) {
  ...
  if (featured) {
    if (legCount !== FEATURED_LEG_COUNT) return []
    pool = pool.filter((bet) => isFeaturedQualityLeg(bet))
  }
  ...
  if (!featured) {
    ...
    parlays = filterFeaturedParlays(parlays)
      .filter((parlay) => matchesFeaturedRequestedType(parlay, type))
  }
