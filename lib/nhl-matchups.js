...
export async function analyzeNHLMatchup(gameId) {
  try {
    ...
    // If no historical data, fetch current season stats from ESPN
    let homeAdvantages, awayAdvantages
    
    if (homeOffenseVsAwayDefense.length === 0 || awayOffenseVsHomeDefense.length === 0) {
      // Fallback to current season stats
      // Use current game's ESPN ID if available (works even for scheduled games!)
      console.log(`🏒 Fetching season stats for ${game.home.abbr} vs ${game.away.abbr}`)
