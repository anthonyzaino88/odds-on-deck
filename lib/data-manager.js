 ...
 * Use this for initial page load, then refresh in background
 * MINIMAL: Only basic game data, no nested relations
 */
export async function getFastData() {
  try {
    console.log('⚡ Getting FAST data (games only - minimal)...')
    
    const { start, end } = getTodaysGamesRange()
    
    // Get games with ONLY home/away teams - no lineups, odds, or edges
    ...
    const results = await Promise.allSettled([
      ...
      // NFL games - minimal (extend to full week)
      (async () => {
        ...
        return prisma.game.findMany({
          ...
          select: {
            ...
            nflData: {
              select: { quarter: true, timeLeft: true }
            }
          },
          orderBy: { date: 'asc' }
        })
    ...
    // Extract results with fallback to empty arrays on error
    const mlbGames = results[0].status === 'fulfilled' ? results[0].value : []
    const nflGames = results[1].status === 'fulfilled' ? results[1].value : []
    const nhlGames = results[2].status === 'fulfilled' ? results[2].value : []
    ...
    return {
      ...
      nflGames,
      nhlGames,
      picks: [], // Will be populated in background
      playerProps: [], // Will be populated in background
      lastUpdated: new Date(),
      isFast: true
    }
  } catch (error) {
    console.error('❌ Error getting fast data:', error)
    // Return empty arrays instead of throwing - page should still render
    return {
      mlbGames: [],
      nflGames: [],
      nhlGames: [],
