...
async function getTodaysMLBGames() {
  // Get games for today accounting for timezone differences
  // Late games (like 9 PM ET) appear as next day in UTC, so we need to extend the range
  const today = new Date()
  today.setHours(0, 0, 0, 0)
  const tomorrow = new Date(today)
  tomorrow.setDate(tomorrow.getDate() + 1)
  tomorrow.setHours(6, 0, 0, 0) // Include games up to 6 AM next day (catches late-night games)
 ...
 * FIXED: Use wider date range to handle timezone differences and show recent/upcoming games
 */
async function getTodaysNHLGames() {
  const now = new Date()
  
  // Include games from yesterday through tomorrow to handle timezones and show active games
