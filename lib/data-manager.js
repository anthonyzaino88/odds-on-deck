...
// Cache for data in memory (persists across requests within the same Vercel function)
const gamesCacheByType = {
  mlb: { data: [], timestamp: 0, ttl: 5 * 60 * 1000 }, // 5 min
  nfl: { data: [], timestamp: 0, ttl: 5 * 60 * 1000 },
  nhl: { data: [], timestamp: 0, ttl: 5 * 60 * 1000 }
}

/**
 * Get FAST data - games and scores only (no heavy API calls)
 * Uses in-memory cache to avoid querying database on every page load
 * Only queries database if cache is expired (5 minutes)
 */
export async function getFastData() {
  try {
    console.log('⚡ Getting FAST data (from cache or database)...')
    
    const now = Date.now()
    const { start, end } = getTodaysGamesRange()
    
    // Check if cache is still valid for each sport
    const isMlbCacheValid = (now - gamesCacheByType.mlb.timestamp) < gamesCacheByType.mlb.ttl
    const isNflCacheValid = (now - gamesCacheByType.nfl.timestamp) < gamesCacheByType.nfl.ttl
    const isNhlCacheValid = (now - gamesCacheByType.nhl.timestamp) < gamesCacheByType.nhl.ttl
    
    // If all caches are valid, return from cache
    if (isMlbCacheValid && isNflCacheValid && isNhlCacheValid) {
      console.log(`✅ Using cached data: ${gamesCacheByType.mlb.data.length} MLB, ${gamesCacheByType.nfl.data.length} NFL, ${gamesCacheByType.nhl.data.length} NHL`)
      return {
        mlbGames: gamesCacheByType.mlb.data,
        nflGames: gamesCacheByType.nfl.data,
        nhlGames: gamesCacheByType.nhl.data,
        picks: [],
        playerProps: [],
    ...
    // Otherwise, query database for expired caches
    console.log('🔄 Cache expired, querying database...')
    
    const results = await Promise.allSettled([
      // MLB games - only if cache expired
      isMlbCacheValid ? Promise.resolve(gamesCacheByType.mlb.data) : prisma.game.findMany({
        where: {
        ...
        select: {
          ...
          homeId: true, awayId: true, sport: true,
          home: { select: { id: true, name: true, abbr: true } },
          away: { select: { id: true, name: true, abbr: true } }
        },
        orderBy: { date: 'asc' }
      }),
      
      // NFL games - only if cache expired
      isNflCacheValid ? Promise.resolve(gamesCacheByType.nfl.data) : (async () => {
        ...
        return prisma.game.findMany({
          where: { date: { gte: weekStart, lt: weekEnd }, sport: 'nfl' },
          select: {
            id: true, date: true, status: true, homeScore: true, awayScore: true,
            homeId: true, awayId: true, sport: true,
            home: { select: { id: true, name: true, abbr: true } },
            away: { select: { id: true, name: true, abbr: true } },
            nflData: { select: { quarter: true, timeLeft: true } }
          },
          orderBy: { date: 'asc' }
        })
      })(),
      
      // NHL games - only if cache expired
      isNhlCacheValid ? Promise.resolve(gamesCacheByType.nhl.data) : prisma.game.findMany({
        where: { date: { gte: start, lt: end }, sport: 'nhl' },
        select: {
          id: true, date: true, status: true, homeScore: true, awayScore: true,
          homeId: true, awayId: true, sport: true,
          home: { select: { id: true, name: true, abbr: true } },
    ...
    const mlbGames = results[0].status === 'fulfilled' ? results[0].value : gamesCacheByType.mlb.data
    const nflGames = results[1].status === 'fulfilled' ? results[1].value : gamesCacheByType.nfl.data
    const nhlGames = results[2].status === 'fulfilled' ? results[2].value : gamesCacheByType.nhl.data
    
    // Update cache with new data
    if (results[0].status === 'fulfilled') {
      gamesCacheByType.mlb.data = mlbGames
      gamesCacheByType.mlb.timestamp = now
    }
    if (results[1].status === 'fulfilled') {
      gamesCacheByType.nfl.data = nflGames
      gamesCacheByType.nfl.timestamp = now
    }
    if (results[2].status === 'fulfilled') {
      gamesCacheByType.nhl.data = nhlGames
      gamesCacheByType.nhl.timestamp = now
    }
    
    // Log any errors
    if (results[0].status === 'rejected') {
      console.error('❌ Error fetching MLB games:', results[0].reason)
    }
    if (results[1].status === 'rejected') {
      console.error('❌ Error fetching NFL games:', results[1].reason)
    }
    if (results[2].status === 'rejected') {
      console.error('❌ Error fetching NHL games:', results[2].reason)
    }
    
    console.log(`✅ Fast data loaded: ${mlbGames.length} MLB, ${nflGames.length} NFL, ${nhlGames.length} NHL`)
    ...
    console.error('❌ Error getting fast data:', error)
...
async function getTodaysMLBGames() {
  // Reduced logging to prevent excessive operations
  const { start, end } = getTodaysGamesRange()
 ...
 * Uses Eastern Time to determine "today"
 * Filters out completed games from yesterday
 */
async function getTodaysNHLGames() {
  const { start, end } = getTodaysGamesRange()
  
  // Import date utilities
  const { getTodayRange } = await import('./date-utils.js')
  ...
  // Filter: keep games that are either:
  // 1. Scheduled for today (Oct 31) or later
  // 2. Currently in progress (even if from yesterday)
  const games = allGames.filter(game => {
    const gameDate = new Date(game.date)
    const isFromToday = gameDate >= startOfToday
    const isLive = game.status === 'in_progress' || game.status === 'warmup'
    
    return isFromToday || isLive
  })
