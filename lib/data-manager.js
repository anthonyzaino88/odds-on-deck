...
async function getTodaysNHLGames() {
  ...
  console.log(`🔍 NHL: Today starts at ${startOfToday.toISOString()} (ET)`)
  ...
  console.log(`✅ NHL: Found ${allGames.length} games in range, ${games.length} relevant for today`)
  console.log(`   - Live games: ${games.filter(g => g.status === 'in_progress').length}`)
  console.log(`   - Scheduled today: ${games.filter(g => new Date(g.date) >= startOfToday && g.status === 'scheduled').length}`)
  return games
}
