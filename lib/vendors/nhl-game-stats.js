...
async function fetchPPPFromPlayByPlay(nhlGameId, playerName) {
  try {
    // Fetch play-by-play and boxscore in parallel (boxscore gives us player ID → name mapping)
    ...
    // Build player ID → name map from boxscore roster
    const playerIdMap = {}
    const positions = ['forwards', 'defense', 'goalies']
    for (const side of ['homeTeam', 'awayTeam']) {
      const team = boxData?.playerByGameStats?.[side]
      if (!team) continue
      for (const pos of positions) {
        for (const p of team[pos] || []) {
          if (p.playerId && p.name?.default) {
            playerIdMap[p.playerId] = p.name.default
          }
        }
      }
    }
    ...
    for (const play of plays) {
      ...
      // Determine if this is a power play goal
      ...
      // Check if our player was scorer or assist on this PP goal
      const involvedIds = [
      ...
      for (const pid of involvedIds) {
        ...
        if (name && matchPlayerName(name, playerName)) {
          ppp += 1
          break // only count once per goal even if somehow listed twice
        }
      }
    }
