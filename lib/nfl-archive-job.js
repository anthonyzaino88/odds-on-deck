...
export function parseArchiveNflArgs(argv) {
  ...
  // Date-only --from/--to stay strings (YYYY-MM-DD). filterCompletedNflEvents
  // treats them as inclusive UTC days; do not Date.parse them here.
...
export function seasonTypeWeeks({ includePreseason, includePostseason, week } = {}) {
  // Bare `archive:nfl` (no --week / --from / --to) walks every regular-season
  // week and the job re-fetches each completed summary. Identical hashes skip
  // the write; use --week or --from/--to to bound a postgame run.
  const types = []
...
export async function archiveNflEvent(eventId, {
  ...
  const normalized = parseEspnNflSummary(raw, { fetchedAt, discovered })
  if (!normalized) {
...
export async function runNflBoxScoreJob(opts = {}, deps = {}) {
  ...
  for (const event of expectedEvents) {
    ...
    const latest = latestIndexRow(loadNflArchiveIndex(archiveRoot).records, eventId)
...
export function formatNflCoverageReport(result) {
  ...
  const lines = [
    ...
    `Healthy archives:    ${c.complete_archives}`,
    `Missing games:       ${(c.missing_games || []).length}`,
    `Corrupt archives:    ${(c.corrupt_archives || []).length}`,
    `Incomplete archives: ${(c.incomplete_archives || []).length}`,
    `Missing files:       ${(c.missing_files_archives || []).length}`,
  ...
  if (c.from || c.to) lines.splice(3, 0, `Date range:          ${c.from || '?'} .. ${c.to || '?'}  (inclusive UTC calendar days)`)
  if (result.indexCorrupt) lines.push(`Corrupt index lines: ${result.indexCorrupt}`)

  const listBlock = (title, rows, lineFor) => {
    if (!rows?.length) return
    lines.push(title)
    for (const row of rows) lines.push(lineFor(row))
  }

  listBlock('Missing:', c.missing_games, (g) => `  - ${g.provider_event_id}  week=${g.week ?? '?'}  ${g.game_time || ''}  ${g.name || ''}`)
  listBlock('Corrupt:', c.corrupt_archives, (g) => `  - ${g.provider_event_id}  v${g.version ?? '?'}  ${(g.issues || []).map((i) => i.kind).join(',')}`)
  listBlock('Incomplete:', c.incomplete_archives, (g) => `  - ${g.provider_event_id}  v${g.version ?? '?'}  ${(g.issues || []).map((i) => i.kind).join(',')}`)
  listBlock('Missing files:', c.missing_files_archives, (g) => `  - ${g.provider_event_id}  v${g.version ?? '?'}  ${(g.issues || []).map((i) => i.kind).join(',')}`)
  listBlock('Failed fetches:', c.failed_fetches, (f) => `  - ${f.status || ''} ${f.url}  ${f.error}`)
  listBlock('Unresolved identities:', c.unresolved_identities, (u) =>
    `  - ${u.provider_event_id} players=${(u.unresolved_player_ids || []).length} teams=${(u.unresolved_team_ids || []).length}`
  )
  return lines.join('\n')
}

export function nflArchiveJobExitCode(result) {
  if (!result) return 2
  if (result.indexCorrupt) return 2
  if (nflCoverageHasFailure(result.coverage)) return 2
  const incompleteWrites = (result.writes || []).some(
    (w) => w.reason === 'incomplete_observation' || w.reason === 'not_completed_from_source' || w.reason === 'unparseable'
  )
  if (incompleteWrites) return 2
  return 0
}
