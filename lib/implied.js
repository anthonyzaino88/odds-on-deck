...
export function decimalToAmerican(decimalOdds) {
  if (!decimalOdds || decimalOdds <= 1) return decimalOdds

  if (decimalOdds >= 2) {
    return Math.round((decimalOdds - 1) * 100)
  }

  // Decimal between 1 and 2 indicates favorite => negative American odds
  return -Math.round(100 / (decimalOdds - 1))
}

export function normalizeToAmericanOdds(odds) {
  if (typeof odds === 'string') odds = parseFloat(odds)
  if (typeof odds !== 'number' || Number.isNaN(odds)) {
    return odds
  }

  // Decimal odds are typically 1.01 to ~50.0 (American equivalent: -10000 to +4900)
  // American odds are typically >= 100 or <= -100
  // Safe heuristic: values between 1 and 50 (exclusive) are decimal
  if (odds > 1 && odds < 50) {
...
/**
 * Estimated expected return at decimal odds d:
 *   P(win) × (d − 1) − P(loss)
 * A refunded push is omitted (contributes 0). This is the game-line
 * formula; calculateEV() above remains the player-prop helper and still
 * treats 1 − P(win) as the losing mass.
 */
export function estimatedEvAtDecimalOdds(pWin, pLoss, decimalOdds) {
  const d = Number(decimalOdds)
  const win = Number(pWin)
  const loss = Number(pLoss)
  if (!Number.isFinite(d) || d <= 1) return null
  if (!Number.isFinite(win) || !Number.isFinite(loss)) return null
  return win * (d - 1) - loss
}

/**
 * Format odds for display (handles both decimal and American input)
 * @param {number} odds - Decimal or American odds
 * @returns {string} Formatted American odds string
 */
export function formatOdds(odds) {
  if (!odds || odds === 0) return 'N/A'
  const american = normalizeToAmericanOdds(odds)
  if (typeof american !== 'number' || isNaN(american)) return 'N/A'
