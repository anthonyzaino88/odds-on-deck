 ...
 * @param {boolean} force - If true, validate all pending props regardless of game status
 */
export async function validateCompletedMockProps(force = false) {
  console.log(`🔍 Validating completed mock props... (force: ${force})`)
  ...
  for (const prop of pendingProps) {
    try {
      ...
      // Check if game is final (skip check if force is true)
      if (!force) {
