...
export async function recordPropPrediction(prop, source = 'system_generated', parlayId = null) {
  try {
    ...
    // Prefer the score that already cleared a public bar (cache / board).
    // Only recompute when the incoming prop has no finite qualityScore.
...
export async function recordGameLinePrediction(line) {
  try {
    ...
    if (String(line.sport || '').toLowerCase() === 'nfl' && line.eligibleForPublic !== true) {
      return null
    }
...
function publishedIdentityMatch(row, fields) {
  return String(row?.gameIdRef || '') === String(fields.gameIdRef)
    && String(row?.playerName || '') === String(fields.playerName)
    && String(row?.propType || '').toLowerCase() === String(fields.propType || '').toLowerCase()
    && Number(row?.threshold) === Number(fields.threshold)
    && String(row?.prediction || '').toLowerCase() === String(fields.prediction || '').toLowerCase()
}

async function findExistingPublishedValidation(fields) {
  const { data: byPropId, error: byIdError } = await supabase
    .from('PropValidation')
    .select('*')
    .eq('propId', fields.propId)
    .maybeSingle()
  ...
  const { data: matches, error } = await supabase
    .from('PropValidation')
    .select('*')
    .eq('gameIdRef', fields.gameIdRef)
    .eq('playerName', fields.playerName)
    .eq('propType', fields.propType)
    .eq('threshold', fields.threshold)
    .eq('prediction', fields.prediction)
    .eq('status', 'pending')
    .limit(5)

  if (error) throw error
  return (matches || []).find((row) => publishedIdentityMatch(row, fields)) || null
}

/**
 * Persist one Published-eligible player prop so morning validate:all
 * can grade it. Uses the cache fields that cleared the bar. Never writes
 * game lines or edge-0 juice props. Leaves completed rows alone.
 */
export async function recordPublishedEligibleProp(prop) {
  try {
    const fields = toPublishedValidationFields(prop)
    if (!fields) return null

    const { data: game, error: gameError } = await supabase
      .from('Game')
      .select('id, sport')
      .eq('id', fields.gameIdRef)
      .maybeSingle()

    if (gameError) throw gameError
    if (!game) {
      console.warn(`recordPublishedEligibleProp: game ${fields.gameIdRef} not found — skip ${fields.playerName}`)
      return null
    }

    const existing = await findExistingPublishedValidation(fields)
    const plan = publishedValidationWritePlan(existing)
    if (plan === 'skip') return existing
    ...
    if (plan === 'update') {
      const { data, error } = await supabase
        ...
        .update({
          ...
          projectedValue: fields.projectedValue,
          ...
          notes: fields.notes,
          ...(fields.numBooks != null ? { numBooks: fields.numBooks } : {}),
        })
        .eq('id', existing.id)
        .select()
        .single()
    ...
    console.error('Error recording Published prop:', error)
    return null
  }
}

/**
 * Record every Published-eligible cache / board row. Idempotent.
 * Does not fetch odds. Does not grade — validate:all does that later.
 */
export async function persistPublishedEligibleProps(props) {
  const rows = Array.isArray(props) ? props.filter((prop) => isPublishedEligibleProp(prop)) : []
  const saved = []
  for (const prop of rows) {
    const row = await recordPublishedEligibleProp(prop)
...
/**
 * Sweep PlayerPropCache for Published-eligible MLB + NFL props and
 * write them to PropValidation. Used by odds-fetch and the ops script.
 * Includes games that already started today so first-pitch traffic
 * still records the slate. No Odds API calls.
 */
export async function persistPublishedEligibleFromCache() {
  if (!supabase) return []
  ...
  if (error) {
    console.error('persistPublishedEligibleFromCache:', error)
    return []
  }
  return persistPublishedEligibleProps(data || [])
}
