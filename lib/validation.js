...
export async function getValidationStats(options = {}) {
  try {
    ...
    // Calculate ROI using actual recorded odds when available
    // For each bet: win pays (decimalOdds - 1) units, loss costs 1 unit
    let totalUnits = 0
    let betsWithOdds = 0
    ...
    validations?.forEach(v => {
      ...
      const odds = v.odds
      if (odds && odds > 1) {
        // Has decimal odds recorded
        if (v.result === 'correct') {
          totalUnits += (odds - 1) // profit on a win
        } else {
          totalUnits -= 1 // lost the unit
        }
        betsWithOdds++
      } else {
        // Fall back to -110 assumption
    ...
    // Calculate accuracy, ROI, and avgImplied by prop type
    Object.keys(byPropType).forEach(type => {
      ...
      // Average implied probability for this prop type
