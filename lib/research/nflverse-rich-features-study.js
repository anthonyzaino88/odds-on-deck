...
function evaluateTotalsHoldout(rows, calibration) {
  ...
  const paired = pairedTotalsComparable(scored)
  const realSigma = scored.filter((row) => row.sigmaSource === 'prior_season')
  const verdict = beatsClosingMarket({
    logLoss: paired.model.logLoss,
    brier: paired.model.brier,
    primaryRoi: paired.primary.roi,
    marketLogLoss: paired.market.logLoss,
    marketBrier: paired.market.brier,
    usesClosingMarket: false,
  })
...
/**
 * Model vs market totals scores and +EV ROI on the same rows:
 * decisive games that have both a model P(over|decisive) and a de-vig close.
 */
export function pairedTotalsComparable(evaluations) {
  const comparable = (evaluations || []).filter((row) => (
    (row.y === 0 || row.y === 1)
    && Number.isFinite(row.pOverDecisive)
    && Number.isFinite(row.marketPOver)
  ))
  return {
    n: comparable.length,
    model: {
      logLoss: mean(comparable.map((row) => binaryLogLoss(row.y, row.pOverDecisive))),
      brier: mean(comparable.map((row) => brierScore(row.y, row.pOverDecisive))),
    },
    market: {
      logLoss: mean(comparable.map((row) => binaryLogLoss(row.y, row.marketPOver))),
      brier: mean(comparable.map((row) => brierScore(row.y, row.marketPOver))),
    },
    primary: summarizeStakeSeries(comparable.map((row) => row.primaryProfit)),
  }
}
...
function poolTotals(seasons) {
  const evaluations = seasons.flatMap((season) => season.evaluations || [])
  const paired = pairedTotalsComparable(evaluations)
  const verdict = beatsClosingMarket({
    logLoss: paired.model.logLoss,
    brier: paired.model.brier,
    primaryRoi: paired.primary.roi,
  ...
  return {
    ...
    marketLogLoss: paired.market.logLoss,
    marketBrier: paired.market.brier,
    primaryStake: { rule: PREREGISTERED_STAKE.totals, ...paired.primary },
    beatsClosingMarket: verdict,
  }
}
...
Overall: n=${totals.overall?.nEvaluated ?? 0} projected games; **${totals.overall?.nDecisive ?? 0}** decisive games with a closing O/U (the vs-market sample). MAE vs model mean ${fmt(totals.overall?.mae, 2)}, MAE vs closing line ${fmt(totals.overall?.maeVsLine, 2)}, log-loss ${fmt(totals.overall?.logLoss, 4)} vs market ${fmt(totals.overall?.marketLogLoss, 4)}, Brier ${fmt(totals.overall?.brier, 4)} vs market ${fmt(totals.overall?.marketBrier, 4)}, primary +EV ROI ${fmtPct(totals.overall?.primaryStake?.roi)} (n=${totals.overall?.primaryStake?.n ?? 0}). Probability scores and +EV ROI use only that paired sample.
