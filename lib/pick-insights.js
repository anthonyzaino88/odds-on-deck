...
async function generatePlayerPropInsights(pick, game) {
  ...
  // Model confidence -- juice traps are never elite
  const qualityScore = pick.qualityScore || 0
  if (isJuiceTrap(pick)) {
    insights.modelConfidence = `Low - Under-0.5 juice trap, not an elite play`
...
export function generateQuickInsight(pick, game) {
  if (pick.type === 'moneyline') {
    ...
    const sport = String(pick.sport || game?.sport || '').toLowerCase()
    const recordLabel = sport === 'nfl' ? 'season record' : 'recent form'
    
    if (winPct && winPct >= 70) {
      return sport === 'nfl'
        ? `Season record: ${record.wins}-${record.losses}`
        : `Hot streak: ${record.wins}-${record.losses} ${recordLabel}`
    } else if (isHome) {
    ...
    if (winPct && winPct >= 60) {
      return sport === 'nfl'
        ? `Season record: ${record.wins}-${record.losses}`
        : `Solid form: ${record.wins}-${record.losses} recently`
    }
  } else if (pick.type === 'total') {
