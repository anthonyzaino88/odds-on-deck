...
export function calculateQualityScore({ probability, edge, confidence }) {
  ...
  // Scoring formula:
  // - 50% probability (likelihood of winning)
  // - 20% confidence (consistency / multi-book agreement)
  // - 30% edge (line shopping value — real data from cross-bookmaker comparison)
  //
  // Edge is capped at 0.15 (15%) for scoring to prevent outlier distortion.
  const cappedEdge = Math.min(edgeVal, 0.15)
  const normalizedEdge = cappedEdge / 0.15  // Scale 0-15% edge into 0-1 range
  const score = (prob * 0.50 + confVal * 0.20 + normalizedEdge * 0.30) * 100
 ...
 */
export function getQualityTier(score, prop) {
  // Counting-stat under 0.5 is never Elite / high quality on /picks
  if (prop && isJuiceTrap(prop)) {
    return {
      tier: 'longshot',
      label: 'Juice trap',
      color: 'red',
      emoji: 'x',
      description: 'Under-0.5 counting line -- heavy juice, not an elite play'
    }
  }
  if (score >= 70) {
    return {
