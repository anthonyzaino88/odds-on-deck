...
/**
 * Legacy nfl-nhl-v0.1.0 rows and unvalidated NFL heuristics stay off
 * the public board. MLB is unchanged (no eligibleForPublic flag required).
 */
export function isApprovedNflGameLine(record) {
  if (!record || typeof record !== 'object') return false
  if (normalizeSport(record.sport) !== 'nfl') return false
  const modelRun = String(record.modelRun || '')
  if (modelRun === 'nfl-nhl-v0.1.0') return false
  return record.eligibleForPublic === true
}
