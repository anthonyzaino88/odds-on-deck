...
export async function upsertTeam(teamData) {
  try {
    // If no ID provided, try to find existing team by abbr+sport first
    if (!teamData.id) {
...
export async function upsertPlayer(playerData) {
  // Build player object, only including fields that exist
  const playerObj = {
    id: playerData.id,
  ...
  // Only add jerseyNumber if it's provided (column might not exist in all schemas)
  if (playerData.jerseyNumber !== undefined) {
    playerObj.jerseyNumber = playerData.jerseyNumber
  }
...
export async function getGameDetail(gameId) {
  ...
  // Ensure espnGameId is included in the response
  // The * selector should include it, but let's be explicit
  ...
  // Get post-game stats for finished NHL games
  let postGameStats = null
  let postGamePlayerStats = null
  if (data.sport === 'nhl' && data.status === 'final' && data.espnGameId) {
    try {
      const { fetchNHLTeamGameStats, fetchNHLPlayerGameStats } = await import('../lib/vendors/nhl-team-stats.js')
      postGameStats = await fetchNHLTeamGameStats(data.espnGameId)
      postGamePlayerStats = await fetchNHLPlayerGameStats(data.espnGameId)
      
      // Debug: Log what we got
      if (postGamePlayerStats) {
        console.log('✅ Post-game player stats fetched:', {
          skaters: postGamePlayerStats.skaters?.length || 0,
          goalies: postGamePlayerStats.goalies?.length || 0,
          sampleSkaters: postGamePlayerStats.skaters?.slice(0, 2)?.map(p => ({
            name: p.playerName,
            goals: p.goals,
            assists: p.assists,
            points: p.points
          })),
          sampleGoalies: postGamePlayerStats.goalies?.slice(0, 2)?.map(g => ({
            name: g.playerName,
            saves: g.saves
          }))
        })
      } else {
        console.log('⚠️ No post-game player stats returned')
      }
    } catch (error) {
      console.error('Error fetching post-game stats:', error)
      // Don't fail the whole request if stats fetch fails
    }
  }
  ...
  // Normalize the date to ensure it's always an ISO string with timezone
  // Supabase may return dates as strings with or without 'Z', or as Date objects
  // IMPORTANT: Dates without 'Z' are stored as UTC but JavaScript interprets them as local time
  // We need to ensure they're treated as UTC by adding 'Z'
  let normalizedDate = data.date
  if (normalizedDate) {
    if (normalizedDate instanceof Date) {
      normalizedDate = normalizedDate.toISOString()
    } else if (typeof normalizedDate === 'string') {
      // If it's a string without timezone info, add 'Z' to treat as UTC
      // This is critical: dates like "2025-11-06T03:00:00" should be "2025-11-06T03:00:00Z"
      // which represents 3 AM UTC, not 3 AM local time
      if (!normalizedDate.includes('Z') && !normalizedDate.includes('+') && !normalizedDate.match(/[+-]\d{2}:\d{2}$/)) {
  ...
  return {
    ...data,
    date: normalizedDate, // Override with normalized date
    odds: odds || [],
    edges: edgeSnapshots || [], // Alias for backward compatibility
    edgeSnapshots: edgeSnapshots || [],
    nflData,
    lineups: lineups || [],
    postGameStats, // Post-game team stats for finished games
    postGamePlayerStats // Post-game player stats for finished games
  }
}
