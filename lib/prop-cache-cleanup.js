...
export function describeCleanupReadError(err) {
  const status = err?.status ?? err?.statusCode ?? err?.httpStatus ?? err?.cause?.status ?? null
  const code = err?.code ?? err?.error_code ?? err?.cause?.code ?? null
  return {
    name: err?.name || null,
    message: sanitizeCleanupLog(err?.message || err?.details || String(err)),
    status,
    code: code == null ? null : String(code),
    retryable: isRetryableCleanupReadError(err),
  }
}
  ...
  return async function fetchPage({ filter, cursor, pageSize }) {
    ...
    if (error) {
      throw new CleanupReadError(`cleanup read failed: ${error.message}`, {
        ...describeCleanupReadError(error),
