...
/**
 * Copy a known book count onto a write payload. Returns the row unchanged
 * when the source has no count — never invents historical book counts.
 * @param {object} row
 * @param {object} [source]
 * @returns {object}
 */
export function attachNumBooks(row, source = row) {
  if (!row || typeof row !== 'object') return row
  const n = getBookCount(source)
  if (n == null) return row
  return { ...row, numBooks: n }
}
